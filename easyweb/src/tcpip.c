...
static unsigned short ISNGenHigh;                // upper word of our Initial Sequence Number
...
static unsigned short RecdFrameLength;           // EMAC reported frame length
static unsigned short RecdFrameMAC[3];           // 48 bit MAC
static unsigned short RecdFrameIP[2];            // 32 bit IP
static unsigned short RecdIPFrameLength;         // 16 bit IP packet length

// the next 3 buffers must be word-aligned!
// (here the 'RecdIPFrameLength' above does that)
static unsigned short _TxFrame1[(ETH_HEADER_SIZE + IP_HEADER_SIZE + TCP_HEADER_SIZE + MAX_TCP_TX_DATA_SIZE)/2];
static unsigned short _TxFrame2[(ETH_HEADER_SIZE + MAX_ETH_TX_DATA_SIZE)/2];
static unsigned short _RxTCPBuffer[MAX_TCP_RX_DATA_SIZE/2]; // space for incoming TCP-data
...
static unsigned short TxFrame1Size;              // bytes to send in TxFrame1
...
const unsigned char* TCP_TX_BUF    =  ((unsigned char *)TxFrame1 + ETH_HEADER_SIZE + IP_HEADER_SIZE + TCP_HEADER_SIZE);
const unsigned char* TCP_RX_BUF    =  ((unsigned char *)RxTCPBuffer);
